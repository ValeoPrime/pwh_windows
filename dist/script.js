...
var forms = function forms(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); //Фильтрация ввода, регулярка заменяет все кроме цифр на пустое место
  ...
  AllForms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //Блок со статусом отправки данных из формы

      var messageStatus = document.createElement('div');
      messageStatus.classList.add('status');
      item.appendChild(messageStatus); //end

      console.log('Атрибут айтема', item.getAttribute("data-calc"));
      var formData = new FormData(item);

      if (item.getAttribute('data-calc') === 'end') {
        // Когда отправляем последнюю форму закидываем в форм дата обьект собранных данных
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    var overleyClose = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
  ...
  bindModal('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close', false, false); // showPopUpByTime('.popup', 60000)
};
      ...
      item.addEventListener(event, function () {
        if (item.nodeName == 'SPAN') {
          ...
          if (item.getAttribute('type') === 'checkbox') {
            ...
            elems.forEach(function (box, c) {
              //Сбрасываем все чекбоксы кроме нажатого в фолс
              box.checked = false;
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    if (e.target.classList.contains(tabSelector.slice(1)) || e.target.parentNode.classList.contains(tabSelector.replace(/\./, ''))) {
      //Убираем точку функцией реплейс иначе контаинс не подхватит название класc либо можно реализовать через слайс
